...
//#define HADM_RTT_TYPE_3

#define ABS(x) (((x) > 0) ? ((uint32_t)(x)) :((uint32_t) -(x)))

/* Approximation of complex magnitude (sqrt(i^2+q^2)). abs_i and abs_q are absolute values */
#define MAG(abs_i,abs_q) (MAX((abs_i),(abs_q)) + (3*(MIN((abs_i),(abs_q))) >> 3U))
...
#define FLAGS_HADM_START_TIMES_PASSED           0x0800          /*!< CS subevent start time happened in the past */
/*! @}*/
...
typedef struct
{
    ...
    bool                   is_config_valid;     /*!< validity flag for HADM configuration */
    ...
    uint8_t                nb_rtt_valid;        /*!< number of valid RTT packets received during the event */
    ...
    uint32_t               time_to_adj;         /*!< Time before drift compensation (in us) */
    uint32_t               time_elapsed;        /*!< Time elapsed since HADM event start or last time adjustment (in us) */
    uint16_t               *ts_buff_tmp_p;      /*!< pointer on buffer where RTT timestamp difference are stored */
...
typedef struct
{
    ...
    int32_t                cfo;                 /*!< CFO [Hz] measured during during alignment phase*/
    hadm_dc_resid_t        dc_residual[HADM_NUM_SUBEVENTS_MAX]; /*!< structure to store DC-residual meas for each subevent */
} hadm_proc_t;
...
typedef struct
{
    ...
    uint8_t                ant2gpio[HADM_MAX_NB_ANTENNAS+1U]; /*!< pointer to the LUT performing mapping btw antenna index an GPIOs */
...
BLE_HADM_STATUS_t lcl_hadm_run_measurement(const BLE_HADM_SubeventConfig_t *hadm_config);
