...
#define LCL_HAL_COMPUTE_CHANNEL_CFO(channel, ppm) \
        ((((int32_t)2402 + (int32_t)(channel)) * (int32_t)(ppm)) / ((int32_t)HADM_PPM_DIVIDER)) /* in Hz */
...
#define LCL_HAL_ENABLE_FO_ENTRY(tx) \
    {\
        XCVR_MISC->IPS_FO_ADDR[HADM_FO_ENTRY] |= (tx) ? XCVR_MISC_IPS_FO_ADDR_ENTRY_TX_MASK:XCVR_MISC_IPS_FO_ADDR_ENTRY_RX_MASK; \
    }

#define LCL_HAL_DISABLE_FO_ENTRY() \
    {\
        XCVR_MISC->IPS_FO_ADDR[HADM_FO_ENTRY] &= ~(XCVR_MISC_IPS_FO_ADDR_ENTRY_TX_MASK|XCVR_MISC_IPS_FO_ADDR_ENTRY_RX_MASK); \
    }

/* Initilize FOM to write to XCVR_PLL_DIG_PLL_OFFSET_CTRL. Trigger is controlled by LCL_HAL_ENABLE_FO_ENTRY/LCL_HAL_DISABLE_FO_ENTRY */
#define LCL_HAL_SET_PLL_OFFSET_FO_ENTRY() \
    {\
        uintptr_t reg_addr = (uintptr_t)&(XCVR_PLL_DIG->PLL_OFFSET_CTRL); \
        XCVR_MISC->IPS_FO_ADDR[HADM_FO_ENTRY] = XCVR_MISC_IPS_FO_ADDR_ADDR(reg_addr); \
    }
