...
typedef void (*emgr_idle_cbk)(void);
 ...
 */
void emngr_register_idle_cbk(emgr_idle_cbk cbk);

/**
 * @brief Used to unblock the event handler loop in case it's blocked after a relevant
 * event occurs
 */
void emngr_signal_event(void);

/**
 * @brief  Evaluate the SW and HW Idle state machine. Calls idle callback if registered
 */
void eval_sw_and_hw_idle_sm();
