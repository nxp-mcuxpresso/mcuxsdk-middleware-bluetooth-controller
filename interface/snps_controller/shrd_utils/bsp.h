 ...
 * @param prio_level  set the current interrupt priority level
 *                where MAX priority = 0, Min priority = NUM_PRIO_LEVEL - 1
 *
 * @retval None
 */
void  set_irq_prio_lvl(int32_t prio_level);

/**
 * @brief  restore the default interrupt priority level
 * @param   None
 *
 * @retval None
 */
void restore_irq_prio_lvl(void);
...
/**
 * @brief a function to check if the upper layer prefer to persist the tx power limits or not.
 *
 * @retval persist : TRUE  -> The upper layer prefer to persist the tx power limits.
 * 					 FALSE -> The upper layer prefer not to persist the tx power limits.
 */
uint8_t bsp_channel_tx_power_limit_is_persistent(void);
/**
 * @}
 */
 ...
 *
 * @param persistent[in]: 0 no persistent tx power limitation
 *         other: persistent tx power limitation
 */
