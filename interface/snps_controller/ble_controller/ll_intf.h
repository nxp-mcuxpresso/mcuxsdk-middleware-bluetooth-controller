...
#endif /* SUPPORT_CONNECTED_ISOCHRONOUS || BROADCAST_ISOCHRONOUS */
...
/**
 * @brief run the LE transmitter test modulated CW.
 *
 * @param channel        : [in] Channel where the wave will be transmitted
 * @param payload_type   : [in] Packet payload type in each packet as in Core Spec (7.8.29).
 * @param phy            : [in] Modulation/Phy to use
 *
 * @retval ble_stat_t       : Command status to be sent to the Host.
 */
ble_stat_t ll_intf_le_transmitter_test_modulated_cw(uint8_t tx_channel,
        uint8_t packet_payload, tx_rx_phy_e phy);
 ...
 *
 * @param  ptr_min_tx_pwr	: [out] A pointer to the min TX power value to be set by the controller [Range: -128.0 dB (0xFB00) -> 128.0 dB (0x0500)]
 * @param  ptr_max_tx_pwr	: [out] A pointer to the max TX power compensation value to be set by the controller [Range: -128.0 dB (0xFB00) -> 128.0 dB (0x0500)]
 *
 * @retval ble_stat_t	: Command status.
 */
...
/*===============  LE Set Connection Initial Transmit Power Level  ===============*/
/**
 * @brief  Set the initial connection TX_Power level value for a certain PHY.
 *
 * @param  phy_type	: [in] PHY type for which the TX_Power value will be set
 * @param  tx_power	: [in] TX_Power level value
 *
 * @retval ble_stat_t	: Command status.
 */
ble_stat_t ll_intf_set_conn_initial_tx_power(tx_rx_phy_e phy_type, int8_t tx_power);

/*=================== Immediate disconnect Command ===============*/
/**
 * @brief  Used to immediately terminate an existing connection without sending LL_TERMINDATE_IND to the peer.
 *
 * @param  conn_handle_id : [in] Connection_Handle for the connection being disconnected.

 * @retval ble_stat_t : Command status to be sent to the Host.
 */
ble_stat_t ll_intf_immediate_disconn(uint16_t conn_handle_id);
...
ble_stat_t ll_intf_le_get_remaining_time_for_next_event_unsafe(uint32_t* remaing_time);
...
/*=================== fix primary and secondary logical channels ===============*/
/**
 * @brief  Used to Force the LL to use only one fixed logical primary and secondary channel.
 *
 * @param  primary_ch : [in] Primary physical channel to be used for adv and scanning.

 * @param  secondary_ch : [in] Secondary channel to be used for connection and extended advertising.

 * @retval ble_stat_t : Command status to be sent to the Host.
 */
ble_stat_t ll_intf_force_fixed_channels(uint8_t primary_ch, uint8_t secondary_ch);

/*=================== limit channels tx power ===============*/
/**
 * @brief  Used to limit tx power of some given channels.
 * This API treat each secondary channel individually but with primary channels limiting tx power
 * of at least one of them will take effect on all three primary channels
 *
 * @param  tx_power_limit : [in] The requested limit of TX power to be used.
 *
 * @param  channels_to_limit_high : [in] the high 32 bit mask for the channels to be masked.
 *
 * @param  channels_to_limit_low : [in] the low 32 bit mask for the channels to be masked.

 * @retval ble_stat_t : Command status to be sent to the Host.
 */
ble_stat_t ll_intf_limit_channels_tx_power(int8_t tx_power_limit , uint32_t channels_to_limit_high, uint32_t channels_to_limit_low);

/*=================== limit phy tx power ===============*/
/**
 * @brief  Used to limit tx power of channels [15, 31] on selected phys .
 *
 * @param  tx_power_limit : [in] The requested limit of TX power to be used.
 *
 * @param  channels_to_limit_high : [in] the high 32 bit mask for the channels to be masked.
 ...
 * @param  channels_to_limit_low : [in] the low 32 bit mask for the channels to be masked.
 *
 * @param  phy_mask       : [in] Bit field of the desired phys to be power limited.
 *
 * @retval ble_stat_t : Command status to be sent to the Host.
 */
ble_stat_t ll_intf_limit_phy_tx(int8_t tx_power_limit , uint32_t channels_to_limit_high, uint32_t channels_to_limit_low, uint8_t phy_mask);
/*=============== LL Start phy calibration command ===============*/
/**
 * @brief	this command is used to start phy calibration and update the mode of subsequent
 *			phy calibration that can take place before starting DTM, the command can only be
 *			used if there is no registered events and DTM is not currently running.
 *
 * @param  mode			: DTM phy calibration mode
 *							0: disable
 *							1: enable
 *
 * @retval ble_stat_t	: Command status.
 */
ble_stat_t ll_intf_start_phy_clbr(uint8_t mode);
...
#endif /* (SUPPORT_BRD_ISOCHRONOUS || SUPPORT_SYNC_ISOCHRONOUS) */
/**
 * @brief  Limit the tx power of the coded phy
 *
 * @param  Enable : [in] The coded phy power limit.
 *
 * @retval ble_stat_t : Command status
 */
ble_stat_t ll_intf_limit_coded_tx_pwr(int8_t pwr_limit);
